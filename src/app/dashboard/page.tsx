...
// --- DECORATIVE ICONS (Using Theme Variables) ---
...
export default async function Dashboard({
  ...
  const currency = user.currency || "₦"; // Default to Naira

  const prevDate = new Date(year, month - 1, 1);
  const prevMonthCount = await prisma.budget.count({
    where: {
      userId: user?.id,
  ...
  const canImport = user?.budgets.length === 0 && prevMonthCount > 0;
  ...
  activeBudgetLimit = viewMode === 'yearly' ? await getYearlyTotalLimit(session.user?.email!, year) : globalLimit;
    ...
    <div className="relative min-h-screen bg-background text-foreground overflow-x-hidden transition-colors duration-500 selection:bg-primary/20">
      ...
      <div className="relative z-10 px-4 py-6 sm:p-8 md:p-10">
        <div className="max-w-5xl mx-auto space-y-6 md:space-y-8">
          ...
          {/* Header Row */}
          <div className="flex max-w-max justify-between items-center bg-card/50 backdrop-blur-sm p-2 rounded-2xl border border-border z-50000">
             <DateFilter />
          </div>

          {/* --- MAIN OVERVIEW CARD --- */}
          <div className="bg-card/80 backdrop-blur-md p-5 sm:p-8 rounded-3xl shadow-xl border border-border/50 ring-1 ring-white/20 z-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
            ...
            <div className="space-y-6 md:space-y-8 order-2 lg:order-1">
              ...
              <section className="bg-card/80 backdrop-blur-sm p-6 rounded-2xl shadow-sm border border-border hover:border-primary/40 transition-all duration-300">
                <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
                    <span className="text-primary text-xl">⚙️</span> Budget Settings
                </h2>
                {/* Note: Update BudgetManager to accept `currencySymbol` props */}
                <BudgetManager 
                        ...
                        <div key={budget.id} className="group">
                          <div className="flex flex-wrap justify-between text-sm mb-2 gap-1">
                            ...
                            <span className={isOver ? "text-red-500 font-bold" : "text-muted-foreground"}>
                              {currency}{spent.toLocaleString()} <span className="opacity-40">/</span> {budget.limit.toLocaleString()}
                            </span>
                          </div>
