  ...
  const TransactionForm = ({ initialData }: { initialData?: any }) => (
    <form onSubmit={handleSave} className="mt-6 space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div>
        ...
        <div className="relative">
          <input 
            ...
            className="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3 outline-none transition-all placeholder:text-slate-300"
          />
        </div>
      </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              placeholder="0.00"
              className="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3 pl-8 outline-none transition-all placeholder:text-slate-300"
            />
          </div>
        </div>
        <div>
          ...
          <input 
            ...
            className="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3 outline-none transition-all"
          />
        </div>
      </div>
      ...
      <div className="relative group">
        ...
        <div className="relative">
          <select 
            ...
            className="w-full appearance-none bg-slate-50 border border-slate-200 text-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3 pr-10 outline-none transition-all"
          >
      ...
      <button 
        type="submit" 
        disabled={saving}
        className="w-full mt-2 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-xl shadow-md shadow-indigo-100 disabled:opacity-50 transition-all transform active:scale-[0.98]"
      >
    ...
    <div className="w-full bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
        <button
          ...
          className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${activeTab === "scan" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          📷 Scan Receipt
        </button>
        <button
          onClick={() => { setActiveTab("manual"); setScanResult(null); }}
          className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${activeTab === "manual" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          ✍️ Manual Entry
        </button>
      </div>
        ...
        <div className="animate-in fade-in duration-300">
          {!scanResult && (
            <>
              ...
              {!preview ? (
                <div 
                  className={`
                    relative border-2 border-dashed rounded-xl p-8 text-center transition-all duration-200 ease-in-out cursor-pointer
                ...
                <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                   ...
                   <div className="p-4 bg-white flex gap-3">
                      <button
                        onClick={handleScan}
                        disabled={isScanning}
                        className="flex-1 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 shadow-md shadow-indigo-100 disabled:opacity-50 transition-all"
                      >
                         {isScanning ? (
                      ...
                      <button 
                        ...
                        className="px-4 py-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg text-sm font-medium transition-all"
                      >
                        Reset
                      </button>
                   </div>
          ...
          {scanResult && (
            <div className="animate-in slide-in-from-bottom-4 fade-in duration-500">
              <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-start gap-3 mb-4">
